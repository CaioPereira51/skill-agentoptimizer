...
const acceptanceTerms = /\b(critérios? de aceite|considerad[oa] (?:pront[oa]|concluíd[oa])|acceptance criteria|done when|validar|teste|test|build|lint|typecheck)\b/i;
...
export const promptQualityRule = {
  ...
  analyze({ sessions }) {
    ...
    for (const session of sessions) {
      ...
      const prompt = textFor(session, "prompts").trim();
      if (!prompt) {
        findings.push(finding(session, this.id, {
          suffix: "missing-objective", category: this.category, severity: Priority.HIGH,
      ...
      if (!objectiveTerms.test(prompt)) findings.push(finding(session, this.id, {
        suffix: "unclear-objective", category: this.category, title: "Objetivo pouco explícito",
      ...
      if (prompt.length >= 80 && !constraintTerms.test(prompt)) findings.push(finding(session, this.id, {
        suffix: "constraints", category: this.category, severity: Priority.LOW, title: "Restrições não identificadas",
      ...
      if (prompt.length < 45 && !constraintTerms.test(prompt) && !acceptanceTerms.test(prompt)) findings.push(finding(session, this.id, {
        suffix: "generic", category: this.category, severity: Priority.HIGH, title: "Prompt excessivamente genérico",
      ...
      if (prompt.length > 12000) findings.push(finding(session, this.id, {
        suffix: "oversized", category: this.category, title: "Prompt muito grande",
      ...
      if (instructionLines.length > 40) findings.push(finding(session, this.id, {
        suffix: "instruction-overload", category: this.category, title: "Excesso de instruções operacionais",
      ...
      if (repeated.length) findings.push(finding(session, this.id, {
        suffix: "repetition", category: this.category, severity: Priority.LOW, title: "Instruções repetidas",
      ...
      if (!acceptanceTerms.test(prompt) && prompt.length > 80) findings.push(finding(session, this.id, {
        suffix: "acceptance", category: this.category, title: "Critério de aceite não identificado",
      ...
      const phaseBoundary = /\b(depois|em seguida|antes de|fase|then|after|before)\b/i.test(prompt);
      ...
      if (contradiction && !phaseBoundary) findings.push(finding(session, this.id, {
        suffix: "contradiction", category: this.category, severity: Priority.HIGH, title: "Instruções potencialmente contraditórias",
      ...
      if (investigate && implement && !phaseBoundary) findings.push(finding(session, this.id, {
        suffix: "mixed-phases", category: this.category, title: "Investigação e implementação sem fronteira clara",
